...
def _get_total_invested_real(acct_no, conn) -> int:
    """stockBalance_stock_balance 에서 trading_plan NOT IN ('i') 기준 총매입금액 조회."""
    try:
        cur = conn.cursor()
        cur.execute("""
            ...
            WHERE acct_no = %s AND proc_yn = 'Y'
              AND (trading_plan NOT IN ('i') OR trading_plan IS NULL)
        """, (str(acct_no),))
        row = cur.fetchone()
        cur.close()
...
def _read_alert_keys_db(conn, acct_no, stock_code, trail_day, trail_dtm, table="public.trading_trail"):
    try:
        cur = conn.cursor()
        cur.execute(f"""
...
def _write_alert_key_db(conn, acct_no, stock_code, trail_day, trail_dtm, key_name, key_value, table="public.trading_trail"):
    try:
        cur = conn.cursor()
        cur.execute(f"""
            UPDATE {table}
...
def get_kis_1min_from_datetime(
    ...
    _mkt_trend_pre  = _get_mkt_trend_real(acct_no, conn)
    ...
    if trail_tp == 'L':
        ...
        for _, row in df.iterrows():
            ...
            if int(proc_min) < int(row['시간'].replace(':', '')+'00'):
                                    ...
                                    if _short_market_down:
                                        ...
                                        if verbose and breakdown_notify_last_key is None:
                                            ...
                                            try:
                                                msg_wait = (
                                                    f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>]"
                                                    f" 15%달성·수익률:{gain_pct:.1f}%, 이탈가({fixed_stop:,}) 이탈 [시장 약세] 매도 진행"
                                                )
                                                print(msg_wait)
                            ...
                            if _short_market_down:
                                ...
                                if verbose and breakdown_notify_last_key is None:
                                    ...
                                    try:
                                        msg_wait = (
                                            f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>]"
                                            f" 수익률:{gain_pct:.1f}%, 이탈가({fixed_stop:,}) 이탈 [시장 약세] 매도 진행"
                                        )
                                        print(msg_wait)
                                        bot.send_message(chat_id=chat_id, text=msg_wait, parse_mode='HTML')
                                    except Exception as te:
                ...
                if (current_time >= _downtrend_start and current_time < _downtrend_warn_end and _trend_down and close_price < _trend_ref_price and row["dt"] == df["dt"].max()):
                        ...
                        if downtrend_warn_last_key is None:
                            ...
                            try:
                                ...
                                _cb_data = f"downtrend_sell:{stock_name}:{stock_code}:{basic_qty}"
                            ...
                            #     _w_invest_base      = _w_mkt_data['tot_evlu_amt']
                            #     _w_allowed_invest = int(_w_invest_base * _w_allow_ratio / 100) if _w_invest_base > 0 else 0
                            #     _w_excess_invest  = _w_total_invested - _w_allowed_invest
                            #     _w_invest_pct     = round(_w_total_invested / _w_invest_base * 100, 1) if _w_invest_base > 0 else 0
                ...
                if (not breakdown_wait["active"] and not sell_trigger and current_time >= _downtrend_cutoff and _trend_down and close_price < _trend_ref_price and not _market_sell_checked and row["dt"] == df["dt"].max()):
                    _market_sell_checked = True

                    print(f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[{stock_code}] [장종료전] 종가:{close_price:,}원 하락추세:{_trend_ref_price:,}원 이탈")
                    ...
                    if not pre_market:
                            ...
                            if trail_tp == '1':
                                ...
                                if breakdown_wait_1["active"]:
                                            ...
                                            try:
                                                ...
                                                if result and verbose:
                                                    try:
                                                        message = (
                                                            f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>]"
                                                            f" [시장 약세] 이탈가 이탈 10분봉 완성 → {sell_label}"
                                                            f" (매도가:{order_price:,})"
                                                        )
                                                        print(message)
                                                        bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                    ...
                                    if breakdown_wait_1["tenmin_low"] is not None and low_price < breakdown_wait_1["tenmin_low"]:
                                        # 해당 종목의 시장이 단기 하락인 경우 : 매도주문가 = 현재가, 매도비율 100%
                                        if _short_market_down:
                                            order_price = close_price
                                            i_trail_plan = trail_plan if trail_plan else "100"
                                        else:   # 해당 종목의 시장이 단기 상승인 경우 : 매도주문가 = 현재가가 이탈가 아래면 이탈가 otherwise 현재가, 매도비율 50%
                                    ...
                                    if breakdown_check <= exit_price and acml_vol > chk_vol:
                                        ...
                                        if _short_market_down:
                                            ...
                                            if breakdown_wait_1["last_alert_tenmin_key"] is None:
                                                ...
                                                try:
                                                    message = (
                                                        f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>] 최종이탈가({exit_price:,})원 이탈 [시장 약세] 10분봉 완성 후 매도 대기"
                                                    )
                                                    print(message)
                                                    bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                                except Exception as te:
                                            ...
                                            if breakdown_wait_1["last_alert_tenmin_key"] is None:
                                                ...
                                                try:
                                                    message = (
                                                        f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>] 최종이탈가({exit_price:,})원 이탈 대기"
                                                    )
                                                    print(message)
                                                    bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                                except Exception as te:
                                    ...
                                    # 이탈가(stop_price) 이탈
                                        ...
                                        if _short_market_down:
                                            ...
                                            if breakdown_wait_1["last_alert_tenmin_key"] is None:
                                                ...
                                                try:
                                                    message = (
                                                        f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>] 이탈가({stop_price:,})원 이탈 [시장 약세] 10분봉 완성 후 매도 대기"
                                                    )
                                                    print(message)
                                                    bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                                except Exception as te:
                                            ...
                                            if breakdown_wait_1["last_alert_tenmin_key"] is None:
                                                ...
                                                try:
                                                    message = (
                                                        f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>] 이탈가({stop_price:,})원 이탈 대기"
                                                    )
                                                    print(message)
                                                    bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                                except Exception as te:
                        ...
                        # ===============================
                                ...
                                if _bw2_is_exit or _bw2_is_stop:
                                    ...
                                    if breakdown_wait_2["last_alert_tenmin_key"] is None or _bw2_key_str > breakdown_wait_2["last_alert_tenmin_key"]:
                                        ...
                                        try:
                                            message = (
                                                f"-{nick}-[{row['일자']}-{row['시간']}]{stock_name}[<code>{stock_code}</code>]"
                                                f" {_bw2_label}({_bw2_breach_price:,})원 분봉 저가 이탈 → 10분봉 완성 후 매도 대기"
                                            )
                                            print(message)
                                            bot.send_message(chat_id=chat_id, text=message, parse_mode='HTML')
                                        except Exception as te:
                            ...
                            if not tenmin_df.empty and row["dt"] == tenmin_df["dt"].max():
                                ...
                                # 기준봉 저가 이탈 여부 무관, 거래량·연속이탈 조건 없이 이탈가 기준으로 즉시 매도 처리
                                # (stop_price >= base_low 인 경우에도 이탈가 이탈 시 매도 보장)
                                ...
                                if sell_trigger:
                                    # 해당 종목의 시장이 단기 하락인 경우 : 매도주문가 = 현재가, 매도비율 70%
                                    if _short_market_down:
                                        order_price = sell_price
                                        i_trail_plan = trail_plan if trail_plan else "70"
                                    else:   # 해당 종목의 시장이 단기 상승인 경우 : 매도주문가 = 기준봉저가가 매도가 아래면 매도가 otherwise 기준봉저가, 매도비율 50%
...
def process_account(nick):
    ...
    try:
        ...
        cur200.execute("select code, name, trail_day, trail_dtm, target_price, stop_price, basic_price, COALESCE(basic_qty, 0), CASE WHEN trail_tp = 'L' THEN 'L' ELSE trail_tp END, trail_plan, proc_min, volumn, trade_tp, exit_price from public.trading_trail where acct_no = '" + str(acct_no) + "' and trail_tp in ('1', '2', 'L') and trail_day = '" + today + "' and to_char(to_timestamp(proc_min, 'HH24MISS') + interval '5 minutes', 'HH24MISS') <= to_char(now(), 'HH24MISS') and basic_qty > 0 order by code, proc_min, mod_dt")
