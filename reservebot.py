        ...
        if menuNum == '41':
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    if was_updated1:
                        conn.commit()
                        context.bot.send_message(chat_id=user_id, text=datetime.now().strftime('%H%M%S')+"[" + company + "{<code>"+code+"</code>}] 추적멈춤 처리", parse_mode='HTML')
                    else:
                        context.bot.send_message(chat_id=user_id, text=datetime.now().strftime('%H%M%S')+"[" + company + "] 추적멈춤 미처리")                        
            except Exception as e:
                conn.rollback()
                print(f"Error 발생: {e}")
                context.bot.send_message(chat_id=user_id, text=f"처리 중 오류가 발생했습니다:{datetime.now().strftime('%H%M%S')} {e}") 
