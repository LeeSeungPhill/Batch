...
def trading_proc(access_token, app_key, app_secret, acct_no):
    ...
    for i, name in enumerate(e.index):
        ...
        insert_query2 = "with upsert as (update dly_stock_balance set buy_qty = %s, sell_qty = %s, purchase_price = %s, purchase_qty = %s, purchase_amt = %s, current_price = %s, open_price = %s, high_price = %s, low_price = %s, volumn = %s, volumn_rate = %s, eval_sum = %s, earnings_rate = %s, valuation_sum = %s, last_chg_date = %s, sign_resist_price = (select sign_resist_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), sign_support_price = (select sign_support_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), end_target_price = (select end_target_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), end_loss_price = (select end_loss_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"') where dt = %s and code = %s and acct = %s returning * ) insert into dly_stock_balance(acct, dt, name, code, buy_qty, sell_qty, purchase_price, purchase_qty, purchase_amt, current_price, open_price, high_price, low_price, volumn, volumn_rate, eval_sum, earnings_rate, valuation_sum, last_chg_date, sign_resist_price, sign_support_price, end_target_price, end_loss_price) select %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, (select sign_resist_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), (select sign_support_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), (select end_target_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"'), (select end_loss_price from \"stockBalance_stock_balance\" where acct_no = '"+acct_no+"' and proc_yn = 'Y' and code = '"+e_code+"') where not exists(select * from upsert)"
        # insert 인자값 설정
    ...
    # 관심정보 조회
    cur21 = conn.cursor()
    cur21.execute("select code, name, through_price, leave_price, resist_price, support_price, trend_high_price, trend_low_price from \"interestItem_interest_item\" where acct_no = '"+acct_no+"'")
