...
if result_one == None:
    ...
    for nick in nickname_list:
        try:
            ...
            sql = """
                SELECT 
                    (SELECT SUM(eval_sum) FROM public."stockBalance_stock_balance" A WHERE A.acct_no = B.acct_no AND (trading_plan NOT IN ('i', 'h') OR trading_plan IS NULL) AND proc_yn = 'Y') - prvs_rcdl_excc_amt
                    , ((SELECT SUM(eval_sum) FROM public."stockBalance_stock_balance" A WHERE A.acct_no = B.acct_no AND (A.trading_plan NOT IN ('i', 'h') OR A.trading_plan IS NULL) AND proc_yn = 'Y') * (100 - market_ratio::numeric) * 0.01)::int - prvs_rcdl_excc_amt
