...
def trading_proc(access_token, app_key, app_secret, acct_no):
    ...
    for i, name in enumerate(e.index):
        ...
        print(f"[{acct_no}] 종목코드 : {e_code} 종목명 : {e_name} 매입단가 : {format_number(e_purchase_price)} 보유수량 : {format_number(e_purchase_qty)} 현재가 : {format_number(e_current_price)} 평가손익 : {format_number(e_valuation_amt)}")
    ...
    for i in result:
        print(f"[{acct_no}] 종목코드 : {i[0]} 종목명 : {i[1]} 돌파가 : {format_number(i[2])} 이탈가 : {format_number(i[3])} 저항가 : {format_number(i[4])} 지지가 : {format_number(i[5])} 추세상단가 : {format_number(i[6])} 추세하단가 : {format_number(i[7])}")
        code = i[0]                                         # 종목코드
        name = i[1]                                         # 종목명
