        ...
        for _, row in df.iterrows():
            ...
            if not breakout_done:
                ...
                if breakdown_check <= stop_price:
                    if verbose:
                        print(f"🚨 [{row['일자']} {row['시간']}] 돌파 전 이탈 발생 → 종료")
            ...
            # ===============================
                ...
                if breakdown_check <= stop_price:
                    if verbose:
                        print(f"🚨 [{row['일자']} {row['시간']}] 돌파 후 이탈 발생 → 종료")
...
def make_10min_candle(df: pd.DataFrame) -> pd.DataFrame:
    """
    1분봉 DataFrame → 10분봉 생성
    """
    df = df.copy()
    ...
    df_10m["시간"] = df_10m["dt"].dt.strftime("%H:%M")
    df_10m["일자"] = df_10m["dt"].dt.strftime("%Y%m%d")

    return df_10m[["일자", "시간", "시가", "고가", "저가", "종가", "거래량", "dt"]]


def get_kis_10min_from_datetime(
    stock_code: str,
    start_date: str,
    start_time: str,
    target_price: int,
    ...
    while current <= today_origin.date() and not process_done:
        ...
        if verbose:
            print(f"[KIS] {stock_code} {trade_date} 10분봉 생성 중")

        # ✅ 1분봉 전체 수집
        df_1m = get_kis_1min_full_day(
            stock_code=stock_code,
            trade_date=trade_date,
            start_time="153000",
            access_token=access_token,
        ...
        # 첫날은 시작시간 이후만
        df_1m["dt"] = pd.to_datetime(
            df_1m["일자"] + df_1m["시간"].str.replace(":", ""),
            format="%Y%m%d%H%M"
        )

        if trade_date == start_date:
            df_1m = df_1m[df_1m["dt"] > start_dt]

        # ✅ 10분봉 생성
        df_10m = make_10min_candle(df_1m)
...
if __name__ == "__main__":
    ...
    # signal = get_kis_10min_from_datetime(
    #     stock_code="287840",
    #     start_date="20251128",
    #     start_time="110000",
    #     target_price=70200,
