...
def trading_proc(access_token, app_key, app_secret, acct_no):
    ...
    for i, name in enumerate(e.index):
        ...
        print(f"[{acct_no}-{e_name}] 보유가 : {format_number(e_purchase_price)}, 보유량 : {format_number(e_purchase_qty)}, 현재가 : {format_number(e_current_price)}, 평가손익 : {format_number(e_valuation_amt)}")
